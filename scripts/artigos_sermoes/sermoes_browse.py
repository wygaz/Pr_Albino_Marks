...
<head>
  ...
  <style>
    ...
    .envbar { margin:-8px -8px 8px -8px; padding:14px 16px; text-align:center; }
    .envbar h2 { margin:0; font-size:24px; line-height:1.2; font-weight:800; letter-spacing:.3px; }
    .envbar.local { background:#8a4b08; color:#fff7ed; }
    .envbar.remoto { background:#0f4c81; color:#eff6ff; }
...
<body>
<div id="envbar" class="envbar remoto"><h2>Banco de Dados Remoto</h2></div>
<div class="topbar">
...
<div class="sw"><button id="ca" class="ctx a">Artigos sem sermao</button><button id="cs" class="ctx">Sermoes formatados</button></div>
<div class="mini"><div class="chip" id="chip">Contexto atual: artigos sem sermao</div><div class="chip" id="targetchip">Alvo BD: remoto</div><div class="chip" id="selinfo">Selecionados: 0 de 0</div></div><div class="summary" id="sum"></div>
...
let ctx='artigos_sem_sermao',sort={key:'pasta_relativa',dir:1},last=[];
...
function metaBox(r){if(ctx==='artigos_sem_sermao'){const pl=articlePlan(r);return `<div><strong>Artigo ID:</strong> <span class="mono">${r.artigo_id||''}</span></div><div><strong>Slug:</strong> <span class="mono">${r.artigo_slug||''}</span></div><div class="small"><strong>BD local:</strong> ${r.publicado_bd_local?'sim':'nao'} | <strong>BD remoto:</strong> ${r.publicado_bd_remoto?'sim':'nao'}</div><div class="small"><strong>Etapa atual:</strong> ${r.etapa_atual||''}</div><div class="small"><strong>Operacao sugerida:</strong> <span class="mono">${pl.label}</span></div><div class="small"><strong>Tipo sugerido:</strong> <span class="mono">${r.publish_kind_recomendado||'all'}</span></div><div class="small"><strong>Obs:</strong> ${r.observacoes||''}</div>`;}const bd=r.artigo_id?`<div class="small"><strong>BD:</strong> Artigo #${r.artigo_id}</div>`:`<div class="small"><strong>BD:</strong> sem match</div>`;return `<div><strong>Slug:</strong> <span class="mono">${r.slug_previsto||''}</span></div>${bd}`;}
