...
FILE_KINDS = {
    'html_a4': ['__A4.html', '__a4.html'],
    'html_a5': ['__A5.html', '__a5.html'],
    'html_tablet': ['__tablet.html'],
    'docx_a4': ['__A4.docx', '__a4.docx'],
    'pdf_a4': ['__A4.pdf', '__a4.pdf'],
    'pdf_a5': ['__A5.pdf', '__a5.pdf'],
...
def normalize_title_from_base(base_name: str) -> str:
    ...
    if '__' in working:
        first, rest = working.split('__', 1)
        first = clean_title_piece(first)
        rest = clean_title_piece(rest)
...
def build_short_label(title: str, base_name: str) -> str:
    # Na interface operacional, o rÃ³tulo deve ficar limpo.
    # A versÃ£o do modelo permanece na heurÃ­stica do nome de arquivo final, nÃ£o no tÃ­tulo visÃ­vel.
    return title
...
def refresh_row_state(row: SermonRow, *, prev_signature: str = '') -> SermonRow:
    ...
    if not row.serie:
        observations.append('Preencher sÃ©rie via BD ou overrides')
    if not row.autor:
        observations.append('Preencher autor via BD ou overrides')
    if row.alterado_desde_ultima_execucao:
        observations.append('Entrada alterada desde a Ãºltima execuÃ§Ã£o')
    current_status = str(row.ultimo_status_execucao or '').strip().upper()
    if row.completo_ok and not row.publicado and current_status in {'', 'PENDENTE'}:
        row.ultimo_status_execucao = 'PRONTO_PUBLICAR'
    elif row.completo_ok and row.publicado and current_status in {'', 'PENDENTE'}:
        row.ultimo_status_execucao = 'OK_UPDATED'
