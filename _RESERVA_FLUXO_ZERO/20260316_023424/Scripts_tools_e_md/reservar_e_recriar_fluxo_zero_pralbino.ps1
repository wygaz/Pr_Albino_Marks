...
function Copy-SelectedToWorkspace {
    ...
    $destDir = Join-Path $WorkspaceRoot $sub
...
function Is-RootOperationalLooseFile {
    ...
    $blockedExact = @(
        ...
        "poetry.lock", "db.sqlite3", $CurrentScriptName
    )

    if ($blockedExact -contains $name) { return $false }

    $allowedExt = @(".py",".ps1",".md",".txt",".docx",".html",".htm",".json",".xlsx")
...
function Is-SelectedForFluxo {
    ...
    # Blocos preservados do fluxo
    if ($p.Contains("\scripts\publicacao\")) { return $true }
    if ($p.Contains("\tools\pralbino_pipeline\")) { return $true }
    if ($p.Contains("\tools\pralbino_sermoes\")) { return $true }
...
$reportRoot = Join-Path $resolvedRoot "_relatorios_saneamento\fluxo_zero_$timestamp"
...
$notesPath = Join-Path $reportRoot "notas_fluxo_zero.txt"
...
# 3) Recria estrutura mínima de trabalho
...
# 4) Restaura só o que interessa para o fluxo zero
Write-Step "Selecionando arquivos do fluxo zero para o workspace"

$selected = New-Object System.Collections.Generic.List[string]
if (Test-Path -LiteralPath $reserveRoot) {
    $candidates = Get-ChildItem -LiteralPath $reserveRoot -Recurse -Force -File | Sort-Object FullName
    foreach ($c in $candidates) {
        if (Is-SelectedForFluxo -FullPath $c.FullName) {
            $selected.Add($c.FullName.Substring($reserveRoot.Length).TrimStart('\'))
            Copy-SelectedToWorkspace -FilePath $c.FullName -ReserveRoot $reserveRoot -WorkspaceRoot $workspaceRoot
        }
    }
}
...
if ($DryRun) {
    ...
    $lines = @("SELECIONADOS PARA Apenas_Local\anexos_filtrados", "Gerado em: $(Get-Date -Format 'yyyy-MM-dd HH:mm:ss')", "") + $selected
    $lines | Set-Content -LiteralPath $selPath -Encoding UTF8
    Write-Host "[OK]  REPORT $selPath" -ForegroundColor Green
}

$notes = @(
    "LIMPEZA GERAL + RESERVA + FLUXO ZERO",
    "Gerado em: $(Get-Date -Format 'yyyy-MM-dd HH:mm:ss')",
    "",
    "Este script:",
    "- move para reserva as zonas operacionais espalhadas",
    "- recria Apenas_Local enxuto",
    "- devolve para Apenas_Local/anexos_filtrados apenas o que interessa ao fluxo zero",
    "",
    ...
    "",
    "Regra operacional pós-limpeza:",
    "- usar Apenas_Local/anexos_filtrados como base dos insumos e scripts de trabalho",
    "- homologar em Apenas_Local/Scripts_Homologados/publicacao",
    "- ao fechar o ciclo, repovoar Apenas_Local/Scripts/publicacao só com homologados"
)
