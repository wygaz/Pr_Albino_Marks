...
$quarantineRoot = Join-Path $rootPath "_quarentena_saneamento\f1_v2_$timestamp"
$reportRoot = Join-Path $rootPath "_relatorios_saneamento\f1_v2_$timestamp"
...
$notesReport = Join-Path $reportRoot "notas_fase1_v2.txt"
...
$zeroFiles = Get-ChildItem -LiteralPath $rootPath -Recurse -Force -File | Where-Object {
    $_.Length -eq 0 -and -not ($skipTopDirs | ForEach-Object { $_ -and $_.Length -gt 0 -and $_.FullName.StartsWith($_, [System.StringComparison]::OrdinalIgnoreCase) } | Where-Object { $_ } | Select-Object -First 1)
}
...
# 4) Notas resumidas
$notes = @(
    "FASE 1 V2 - SANEAMENTO RESPONSAVEL",
...
if ($DryRun) {
    ...
    Write-Host "Fase 1 v2 concluída."
