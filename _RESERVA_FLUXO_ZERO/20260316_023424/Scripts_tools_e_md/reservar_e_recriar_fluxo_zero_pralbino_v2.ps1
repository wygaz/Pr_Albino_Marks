...
# Pré-seleção para o fluxo zero (funciona também no DryRun)
Write-Step "Pré-selecionando candidatos do fluxo zero"
$selectedCandidatePaths = New-Object System.Collections.Generic.List[string]

foreach ($d in $dirTargets) {
    if (Test-Path -LiteralPath $d) {
        Get-ChildItem -LiteralPath $d -Recurse -Force -File | Sort-Object FullName | ForEach-Object {
            if (Is-SelectedForFluxo -FullPath $_.FullName) {
                $selectedCandidatePaths.Add($_.FullName)
            }
        }
    }
}
foreach ($f in $rootLooseFiles) {
    if (Test-Path -LiteralPath $f) {
        if (Is-SelectedForFluxo -FullPath $f) {
            $selectedCandidatePaths.Add($f)
        }
    }
}

$selectedCandidatePaths = $selectedCandidatePaths | Sort-Object -Unique
...
foreach ($src in $selectedCandidatePaths) {
    $selectedLines.Add($src.Substring($resolvedRoot.Length).TrimStart('\'))
    Copy-SelectedToWorkspace -FilePath $src -WorkspaceRoot $workspaceRoot
}
...
$notes = @(
    "LIMPEZA GERAL + RESERVA + FLUXO ZERO (v2)",
