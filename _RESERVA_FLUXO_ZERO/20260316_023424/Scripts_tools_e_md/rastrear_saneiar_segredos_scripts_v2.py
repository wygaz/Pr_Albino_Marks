...
def _abs_pos(line_starts: list[int], line: int, col: int) -> int:
    return line_starts[line - 1] + col


def build_python_protected_ranges(text: str) -> list[tuple[int, int]]:
    """
    Retorna faixas absolutas (start, end) de comentários e strings em código Python.
    Isso evita falso positivo em:
    - comentários
    - literals de string
    - textos como "DATABASE_URL =" dentro de print/log/docstring
    """
    ...
    try:
        ...
        for tok in tokens:
            if tok.type in {tokenize.COMMENT, tokenize.STRING}:
                start = _abs_pos(line_starts, tok.start[0], tok.start[1])
                end = _abs_pos(line_starts, tok.end[0], tok.end[1])
                ranges.append((start, end))
    except tokenize.TokenError:
        # Em arquivo quebrado, falha para aberto: não protege nada e mantém comportamento conservador.
        return []
    return ranges


def is_protected_pos(pos: int, protected_ranges: list[tuple[int, int]]) -> bool:
...
def is_powershell_comment_line(text: str, pos: int) -> bool:
...
def replace_python(text: str, path: Path, findings: list[Finding]) -> str:
    original = text
    protected_ranges = build_python_protected_ranges(text)

    def repl_url(m: re.Match[str]) -> str:
        if is_protected_pos(m.start(), protected_ranges):
            return m.group(0)
        url = m.group("url")
    ...
    def repl_assign(m: re.Match[str]) -> str:
        if is_protected_pos(m.start(), protected_ranges):
            return m.group(0)
        if is_powershell_comment_line(original, m.start()):
            return m.group(0)
        var = m.group("var")
        value = m.group("value")
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Rastreia e sanitiza segredos hardcoded em scripts Python/PowerShell, ignorando comentários e strings inocentes."
    )
