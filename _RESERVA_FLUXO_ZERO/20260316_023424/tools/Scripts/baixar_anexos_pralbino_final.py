...
def main():
    ...
    BASE_DIR = SCRIPTS_DIR.parent              # ...\\anexos_filtrados
    ...
    if not args.nao_consolidar:
        ...
        if resp in ("", "s", "sim"):
            ...
            subprocess.run([sys.executable, str(cons_script), "--lote", OUT_DIR.name], check=False)
            ...
            if not args.nao_prompts:
                ...
                if series_name:
                    ...
                    subprocess.run([sys.executable, str(gen_script), "--series", series_name], check=False)
                else:
