...
function Is-BadNameOrPath {
    ...
    if ($name -ieq "sermoes_browse - .py") { return $true }
    if ($name -ieq "comparar_modelos_sermoes_pralbino.py") { return $true }
    if ($name -ieq "exportar_formatos_sermao_md.py") { return $true }
    if ($name -ieq "normalizar_s3_listagem.py") { return $true }
...
function Get-SelectionScore {
    ...
    if ($name -ieq "README_ETAPA2_ORQUESTRADOR.md") {
        if ($p -notmatch "\\") { $score += 1000 }
        if ($FullPath -match "README_ETAPA2_ORQUESTRADOR\.md$") { $score += 100 }
    }
...
$reportRoot = Join-Path $resolvedRoot "_relatorios_saneamento\fluxo_zero_v4_$timestamp"
...
$notesPath = Join-Path $reportRoot "notas_fluxo_zero_v4.txt"
...
# preferência explícita para o README da raiz
$rootReadme = Join-Path $resolvedRoot "README_ETAPA2_ORQUESTRADOR.md"
if (Test-Path -LiteralPath $rootReadme) {
    try {
        $ri = Get-Item -LiteralPath $rootReadme -Force
        if (-not $ri.PSIsContainer -and $ri.Length -gt 0) {
            $selectedMap["README_ETAPA2_ORQUESTRADOR.md"] = [PSCustomObject]@{
                Name = "README_ETAPA2_ORQUESTRADOR.md"
                Source = $rootReadme
                Score = 9999
            }
        }
...
$notes = @(
    "LIMPEZA GERAL + RESERVA + FLUXO ZERO (v4)",
    ...
    "- exclui auxiliares fora do fluxo imediato",
    "- desclassifica arquivos zerados",
    "- força o README_ETAPA2_ORQUESTRADOR.md da raiz como canônico",
    "",
